...
| **BIN-5 (2026-05-08)** | **350 enlaces externos detectados como missing**. Análisis profundo posterior categoriza así: (a) **~308 falsos positivos del layout global** (ConverTIC, x.com/twitter, funcionpublica.gov.co); de esos, **2 fueron arreglados** (commit `d1ab44a`: twitter→x.com, funcionpublica→/web/sigep/hojas-de-vida) eliminando ~200 falsos positivos; los ~102 restantes son ConverTIC eliminado a propósito (Lote W). (b) **~25 falsos positivos por normalización**: URL local existe pero con protocolo (http/https), www o path distintos al WP origen. (c) **~3 falsos positivos por host alternativo**: WP linkea a presidencia.gov.co/normativa/LEY-1952.pdf, local sirve copia local en `/documentos/normativa/Ley_1952_de_2019.pdf` (mejor). (d) **~5-10 señales reales**: páginas como `historico-sistema-de-control-interno`, `informacion-para-mujeres`, `supervision-y-vigilancia` no tienen sección "Enlaces de interés" con entes relacionados (Contaduría, DAFP, Contraloría, Minhacienda, Procuraduría). Decisión editorial pendiente — agregar cuando se refactoricen estas páginas. (e) **Recomendación**: mejorar `scripts/audit-links.py` para normalizar protocolo + www + fragments antes de comparar (eliminaría ~25 falsos positivos de tipo b). Detalle full en `reports/audit-links-2026-05-08.json`. |
