...
| **BIN-5 (2026-05-08)** | **Iteración inicial**: 350 enlaces externos detectados como missing, 88% falsos positivos por layout repetido. **Iteración refactor (mismo día)**: `scripts/audit-links.py` mejorado con (1) auto-detección de layout-level via threshold 50% de páginas, (2) domain aliases (twitter↔x.com), (3) config opcional para omisiones intencionales y redirects WP→local. **Resultado**: de 350 missing pasó a **44 content-level + 1 layout-level** (ConverTIC, reportado una vez con conteo de 102 páginas). Los 44 restantes son señales reales por revisar: `directorio-de-entidades` (11), `marco-legal` (6), `historico-sistema-de-control-interno` (5), `addtoany.com` en 6 páginas (share buttons sin migrar), enlaces a Minhacienda/Procuraduría/Contaduría/SUIN-Juriscol en páginas específicas. Detalle en `reports/audit-links-2026-05-08.json`. Refactor del script vive solo local (scripts/ gitignored). |
