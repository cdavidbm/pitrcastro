...
| **BIN-5 (2026-05-08)** | **Iteración 1**: 350 missing detectados, 88% falsos positivos por layout. **Iteración 2 (mismo día)**: `scripts/audit-links.py` mejorado con (1) auto-detección de layout-level via threshold 50%, (2) domain aliases (twitter↔x.com), (3) canonicalización de esquema `http→https` y `www.` stripping, (4) domain-match (URL host presente en local cuenta como cubierto), (5) config para `intentionally_omitted` (addtoany, Flash, auth-temporal) y `redirect_map` (Ley 1952 PDF local), (6) parser regex en lugar de `html.parser` (rompía con `<Ver Notas de Vigencia` legal). **Trayectoria**: 350 → 44 → 18 → 10 → 8 → **2 reales restantes** (-99.4%). Resueltos: 5 entes externos en `historico-sistema-de-control-interno` (commit `__PENDING__`); ConverTIC layout-level (decisión Lote W); 14 dominios filtrados por domain-match; 1 redirect Ley 1952 PDF local. **Pendientes editoriales (2)**: `normativa-aplicada` → mininterior PGN-Directiva-020/2022 PDF; `programas-y-proyectos-en-ejecucion` → DNP ficha proyecto PGN. Ambos son PDFs externos; decidir si replicar local o linkear externo. Refactor vive solo local (scripts/ gitignored). |
