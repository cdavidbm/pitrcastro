#!/usr/bin/env node
// Migra los 55 posts de categoría `memorias-eje-educacion` del Observatorio
// WP a páginas internas + regenera el listado por año.
//
// Reutiliza el dump /tmp/lote-b-full.json (con content incluido).
// Aplica D3 (schema Post), D4 (slugs ASCII, truncados a 80) y D5 completo.
//
// Uso: NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-memorias-educacion.mjs
...
const SRC_DUMP = "/tmp/lote-b-full.json";
const OUT_DIR = join(ROOT, "src/content/pages/observatorio/eje-de-educacion/memorias");
const LISTADO_PATH = join(ROOT, "src/content/pages/observatorio/eje-de-educacion/memorias.json");
...
if (broken.length > 0) {
  ...
  writeFileSync(join(REPORTS_DIR, `broken-assets-memorias-educacion-${TODAY}.md`), lines.join("\n") + "\n");
}
...
if (emptyPosts.length > 0) {
  ...
  writeFileSync(join(REPORTS_DIR, `empty-posts-memorias-educacion-${TODAY}.md`), lines.join("\n") + "\n");
}

// Reporte de colisiones de slug resueltas.
const colisiones = [...slugCounts.entries()].filter(([, n]) => n > 1);

console.log(`\n== REPORTE LOTE B — Memorias Eje Educación ==`);
console.log(`Posts procesados:         ${written.length} (esperado 55)`);
...
if (broken.length) console.log(`Broken:  reports/broken-assets-memorias-educacion-${TODAY}.md`);
if (emptyPosts.length) console.log(`Vacíos:  reports/empty-posts-memorias-educacion-${TODAY}.md`);
